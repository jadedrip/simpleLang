...
| Func     | 函数对象                                                     |
| Delay    | 延迟对象                                                     |
| Optional | 可选对象，和 [类名]+? 格式（Clz? clz) 切换                   |
| Delegate | 委托（保留）                                                 |
| Proxy    | 代理（保留）                                                 |
...
以及枚举：
	...
	enum MyEnum{ red=0, blue=2 }	// 枚举和 int 可以手工转换，默认情况下枚举从0开始
...
*设计语：通过将变量和类型使用大小写区分开，可以增强语法分析，避免歧义，禁用下划线是因为编译器会生成带下划线的函数，可以避免冲突。同时这也从语法层面直接规范了风格，提高可读性。*

定义变量可以用 var 来自动推导，用右值推断类型。
...
SimpleLang 语言中支持元组，类似 C++ 中的 pair, tuple。Si 里的元组用圆括号括起来，必须从明确的值创建，并且创建后 **不可增加删除元素**
...
函数返回多个值时，返回内容被视为一个元组，元组被放入函数参数中时，会被视为多个参数

```
var a = (12,"no")
fun("yes", a) // 等价于 fun("yes", 12, "no")
```
...
用 remove 的返回的迭代器重新开始循环体 (continue)

	for( var i : list ){
		free i		// 会调用 list.remove 并重新开始循环
	}
...
返回值如果有非命名参数，那么**必须**写在列表的前面
	...
	func second( int a, var b ) : int, float va1, int va2 {
		return 0, 15.0, 2	
	}
...
上面这个 for 循环会在编译时被展开成顺序的多个代码块。
	...
	func varFunc2( int a, MyClz ... args ){
		//  args 被视为 MyClz 数组
...
函数也可以使用链式调用，前一个函数的返回作为参数，链式调用使用冒号链接

链式调用时，如果参数正好匹配，可以去除括号，如果不匹配，可以使用 $ 符号作为占位符
...
另外，函数可以使用 $ 符号作为参数，来使用参数绑定。使用参数绑定时，被绑定的函数不能是模板的，如果函数是模板的，可以通过带类型的 $ 将它转换为确定类型的函数对象

```
func myFun( int a, int b, int c){ ... } 
var bind = myFun( 12, $, int $ )
bind( 77, 88 )	// 会调用 myFun( 12, 77, 88 )
